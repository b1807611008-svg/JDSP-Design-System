...
description: 先产出 Relay 设计稿 outline 供设计师确认，再生成 design.md。零输入，自动推断 level/bg/slug，自动反查 V16 token，自动维护双向追溯索引。当用户说"把 Relay 稿转成 design.md"、"同步设计稿"、"生成 design.md"、"从 Relay 导出规范"、"把设计稿写进文档"，或传入一个 relay.jd.com URL 并希望生成文档时，必须激活本技能。
allowed-tools: [mcp__zero-design__get_design_metadata, mcp__zero-design__get_design_context, mcp__zero-design__get_screenshot, mcp__zero-design__get_variables, mcp__zero-design__use_design_script, run_in_terminal, read_file, write_file, replace_file]
---
